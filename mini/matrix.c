...
#define JUMP_DELAY 35  //milliseconds between jumps
...
void housekeeping_task_kb(void) {
    ...
    if (timer_elapsed32(jump_timer) > JUMP_DELAY) {
      tap_code(KC_SPC);           // Send an Space keystroke
...
__attribute__((weak)) void keyboard_report_hook(keyboard_parse_result_t const* report) {
    ...
    {
        #if VERBOSE_DEBUG == 1
        uprintf("Keyboard report hook\n");
        dprint("\nr/c 01234567\n");
        for (uint8_t row = 0; row < MATRIX_ROWS; row++)
        {
            xprintf("%02d: ", row);
            print_bin_reverse8(report->bits[row]);
            printf("\n");
        }
        ...
        uprintf("\n");
        #endif
        // Ну и на матрице для всяких info.json
        for (uint8_t row = 0; row < MATRIX_ROWS; row++)
        {
            if (report->bits[row]>0)
            {
                uprintf("matrix_hook [%d, ",row);
                for(int i=7;i>=0;i--)
                {
                    if(report->bits[row]&(1<<i))
                    {
                        uprintf("%d]\n",i);
                        break;
                    }
