...
bool parse_report_descriptor(uint8_t interface, uint8_t const *desc,uint16_t len) {
    ...
    case HID_RI_INPUT(0):
      ...
      for (int idx = 0; idx < usage_table_idx; idx++) {
        ...
        if (new_member == NULL) {
          ...
          dprintf("Register to report table 0x%08X\n", (uintptr_t)new_member);
        }
