...
void keyboard_report_parser(hid_report_member_t const *member,uint8_t const *data, uint8_t len);
void mouse_report_parser(hid_report_member_t const *member, uint8_t const *data,uint8_t len);
void system_report_parser(hid_report_member_t const *member,uint8_t const *data, uint8_t len);
void consumer_report_parser(hid_report_member_t const *member,uint8_t const *data, uint8_t len);
__attribute__((weak)) void vendor_report_parser(uint16_t usage_page, hid_report_member_t const *member, uint8_t const *data,uint8_t len);
...
bool parse_report(uint8_t interface,uint8_t const *report, uint8_t len)
{
  ...
  typedef void (*parser_func_t)(hid_report_member_t const *, uint8_t const *,uint8_t);
  parser_func_t func;
  switch (usage_id)
  {
    case 0x0106:
      dprintf("keyboard_report_parser\n");
      func = keyboard_report_parser;
    break;
    case 0x0102:
      dprintf("mouse_report_parser\n");
      func = mouse_report_parser;
    break;
    case 0x0180:
      dprintf("system_report_parser\n");
      func = system_report_parser;
    break;
    case 0x0C01:
      dprintf("consumer_report_parser\n");
...
static int16_t parse_value(hid_report_member_t const *member, uint8_t const *data, uint16_t *bit_idx) {
  int16_t result = 0;
...
void mouse_report_parser(hid_report_member_t const *member, uint8_t const *data,uint8_t len) {
  ...
  uint16_t bit_idx = 0;
  while (member != NULL) {
    uint32_t usage_id = (((uint32_t)member->global.usage_page) << 16) | member->local.usage;
